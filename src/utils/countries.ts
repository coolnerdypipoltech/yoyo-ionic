// Minimal country + dial-code dataset for the phone number selector.
// Names are not stored here — they're resolved at render time via
// Intl.DisplayNames so they show up translated to whatever language the
// app is currently in, instead of maintaining a second translation table.
...
export const DEFAULT_COUNTRY_ISO2 = 'MX';

export function getCountry(iso2: string): CountryDialCode {
  return COUNTRIES.find((c) => c.iso2 === iso2) ?? COUNTRIES[0];
}

export function dialCodeFor(iso2: string): string {
  return getCountry(iso2).dialCode;
}

// Regional-indicator flag emoji built from the two ISO letters — avoids
// shipping ~190 flag image assets just for this picker.
